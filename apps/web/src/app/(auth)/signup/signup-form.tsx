...
export function SignupForm({
  ...
  betaToken?: string;
  betaPreview?: SignupBetaPreview | null;
}) {
    ...
    <div className="w-full max-w-md rounded-2xl border border-border bg-surface p-8 shadow-[var(--shadow-elevated)]">
      ...
      {betaPreview ? (
        <p className="mt-3 rounded-lg border border-border bg-background px-3 py-2 text-xs text-foreground-muted">
          Beta access for <span className="font-medium text-foreground">{betaPreview.email}</span>.
          No payment required for {betaPreview.betaDays} days.
          {betaPreview.aiEnabled
            ? ` Includes ${betaPreview.aiIncludedUnits.toLocaleString()} AI units during beta.`
            : " AI is disabled during beta."}
        </p>
      ) : (
        <p className="mt-3 rounded-lg border border-border bg-background px-3 py-2 text-xs text-foreground-muted">
      ...
      <form className="mt-8 space-y-4" onSubmit={onSubmit}>
        ...
        <label className="block">
          ...
          <input
            ...
            autoComplete="email"
            required
            defaultValue={betaPreview?.email}
            readOnly={Boolean(betaPreview?.email)}
            className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm text-foreground outline-none transition-colors focus:border-border-strong read-only:opacity-80"
          />
        </label>
        <label className="block">
