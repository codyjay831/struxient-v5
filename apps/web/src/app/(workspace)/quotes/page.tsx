...
import { deriveLeadTitle, readContact } from "@/lib/lead/lead-projection";
...
export default async function QuotesPage({
  ...
  const [
    ...
    db.quote.findMany({
      ...
      include: {
        ...
        lead: { select: { id: true, contact: true, request: true, createdAt: true } },
  ...
  const serializedQuotes: SerializedQuoteListRow[] = rows.map((r) => {
    ...
    const leadTitle = r.lead
      ? deriveLeadTitle(r.lead.contact, r.lead.request)
      : null;
    const leadContactName = r.lead ? readContact(r.lead.contact).name : null;
    const primaryIdentity = leadTitle || r.customer?.displayName || r.title;
    ...
    if (r.lead) {
      const leadBits = [`Lead: ${leadTitle ?? "Lead"}`];
      const cn = leadContactName?.trim();
