...
export function JobVisitManager({ 
  ...
  const [showForm, setShowForm] = useState(initialShowScheduleForm);
  ...
  const displayedUpcoming = isEmbedded && focusId
    ? upcomingVisits.filter((v) => v.id === focusId)
    : upcomingVisits;
  ...
  const scheduleForm = showForm ? (
    <WorkspacePanel className={`border-dashed border-accent/30 bg-accent/5 ${isEmbedded ? "" : "mb-4"}`}>
  ...
  const visitList = (
    <div className="space-y-3">
      {displayedUpcoming.length === 0 && !showForm && !isEmbedded ? (
      ...
      {!isEmbedded && pastVisits.length > 0 && (
        <div className="mt-6">
