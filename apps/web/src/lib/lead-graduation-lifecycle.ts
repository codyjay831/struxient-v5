...
export type SerializedLeadQuoteSummary = {
  id: string;
  title: string;
  statusLabel: string;
...
export type SerializedLeadRowSnapshot = {
  id: string;
  quotes: SerializedLeadQuoteSummary[];
...
/** Reset post-create / graduation client state only when the open lead id changes. */
export function shouldResetLeadWorkspaceState(
...
export function patchSerializedLeadRowAfterQuoteStarted<
  T extends SerializedLeadRowSnapshot,
>(
  row: T,
  args: {
    quoteId: string;
    activeQuotePayload: LeadGraduationActiveQuotePayload | null;
  },
): T {
  const quote = args.activeQuotePayload?.quote;
  if (!quote) return row;

  const quoteSummary: SerializedLeadQuoteSummary = {
    id: quote.id,
    title: quote.title,
