...
export function getOpportunityFlow(input: OpportunityFlowInput): OpportunityFlowView {
  ...
  if (lead.status === "LOST" || lead.status === "ARCHIVED") {
    ...
    return {
      ...
      summary: "Opportunity is marked lost.",
      requirements: [],
      secondaryActions: [],
      primaryAction: null,
      ...resultBase,
  ...
  if (lead.status === "ON_HOLD") {
    ...
    return {
      ...
      summary: lead.followUpAt
        ...
        : "Opportunity is paused until follow-up.",
      requirements: [],
      secondaryActions: [],
      primaryAction: { kind: "RESUME_OPPORTUNITY", label: "Resume opportunity", targetLeadId: lead.id },
      ...resultBase,
    };
  }
  ...
  if (latestOpenChangeRequest) {
    if (openChangeNeedsVisit && !scheduledVisit && !completedVisit) {
      ...
      return {
        ...
        conditionLabel: "Follow-up sales visit required",
        conditionStartedAt,
        ageLabel: formatAge(conditionStartedAt, now),
        summary: "Customer requested changes that need another site visit before revising.",
  ...
  if (scheduledVisit) {
    ...
    return {
      ...
      conditionStartedAt,
      ageLabel: formatAge(conditionStartedAt, now),
      summary: "Site visit is scheduled. Capture measurements and build scope on site.",
      requirements: ["Complete sales visit"],
  ...
  if (pendingVisit) {
    ...
    return {
      ...
      conditionLabel: "Needs sales visit",
      conditionStartedAt,
      ageLabel: formatAge(conditionStartedAt, now),
      summary: "Site visit requested. Schedule the visit, then capture scope on site.",
      requirements: ["Schedule sales visit"],
      secondaryActions: buildDiscoverySecondaryActions(input.quotes, lead.id),
      primaryAction: {
        kind: "SCHEDULE_SALES_VISIT",
        label: "Schedule visit",
  ...
  if (!report.isReady) {
    ...
    return {
      ...
      conditionLabel: "Needs intake details",
      conditionStartedAt,
      ageLabel: null,
      summary: "Request is missing details required to start a quote.",
  ...
  if (!lead.customerId && input.hasExistingCustomerMatch) {
    return {
      ...
      ageLabel: formatAge(conditionStartedAt, now),
      summary:
        "A customer with matching contact info already exists. Review the match before building a quote.",
  ...
  return {
    ...
    conditionStartedAt,
    ageLabel: formatAge(conditionStartedAt, now),
    summary: "Opportunity has enough detail to start quoting.",
    requirements: [],
    secondaryActions: [],
