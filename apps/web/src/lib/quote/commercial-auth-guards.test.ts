...
test("viewer field and subcontractor roles cannot perform commercial mutations", () => {
  assert.equal(
    denyUnlessCanManageCommercial(StaffRole.VIEWER),
  ...
  assert.equal(denyUnlessCanManageCommercial(StaffRole.OWNER), null);
