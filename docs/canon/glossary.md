...
| **Customer Project Portal** | Customer-facing project hub with **controlled disclosure** of project status, next required action, schedule, quotes/change orders, payments, documents/photos, and structured requests. Do not call this product area **Customer Login** yet; authentication is only the access mechanism. Canon: [customer-project-portal-canon.md](./customer-project-portal-canon.md). |
| **Customer Portal Access** | Scoped, revocable customer permission for a specific organization + customer + job/project. It is separate from staff `Membership` and must be enforced server-side. |
| **Customer Portal Identity** | Verified external customer identity used for portal sessions/magic links. It is separate from `CustomerContact`, which describes that person's relationship to a customer/property in an organization. |
| **Customer-visible resource** | Explicit visibility grant for a quote, change order, invoice/payment link, document, photo, schedule event, project update, request, or upload. Internal-only resources do not have a customer-visible row. |
| **Customer portal event** | Append-only audit record for customer-side access and actions, such as portal opened, quote accepted, document uploaded, appointment confirmed, or access revoked. |
...
| **Authorization (authZ)** | **What** a principal may do and **which org’s data** they access—**after** auth. Implemented with **org-scoped resources** + **membership role** (staff) or **scoped portal tokens / customer grants** (customers); **always server-enforced** (I19). Runtime staff detail: [execution-aware-authorization-canon.md](./execution-aware-authorization-canon.md). |
...
| **Clarify Scope** | User-facing quote workflow for structured scope questions and confirmed answers before send. **Single truth path** from unresolved gap → confirmed quote truth—[quote-clarification-canon.md](./quote-clarification-canon.md). |
| **Scope gap** | An unresolved quote question (commercial, selection, site, etc.) tracked internally; may originate from AI suggestion. Must be answered, dismissed, or deferred through Clarify Scope—not via deprecated “Scope Details Needed” triage. |
| **Selection (future)** | Lightweight quote-line product/finish/model choice (not inventory). Documented direction only in clarification canon; not a full material system. |
| **Scope Details Needed** | **Deprecated** user-facing label. Do not use in new UI. Internal records may persist as `QuoteScopeDecision` during cleanup. |
...
| **Change order (CO)** | Post-approval **commercial delta + proposed execution delta** against an active job—not a new quote or full plan replacement. Applied via audited `ExecutionPlanRevision` after acceptance; does **not** silently rewrite **approved quote baseline**—[change-order-canon.md](./change-order-canon.md), [locked-decisions-v1.md](./locked-decisions-v1.md) §7. |
| **Execution delta (CO)** | Versioned JSON proposal (`executionDeltaJson`) of MVP ops (`ADD_SCOPE_ITEM`, `ADD_TASK`, …) against `Job.jobPlanVersion` at CO draft; **must not** mutate job until validated apply—[change-order-canon.md](./change-order-canon.md) §6. |
| **Job plan version** | `Job.jobPlanVersion` — monotonic counter for post-activation execution plan revisions; CO stores `baseJobPlanVersion` at draft—[change-order-canon.md](./change-order-canon.md) §3. |
