...
1. **Commercial truth vs execution plan** — A `SENT` or `APPROVED` quote is an immutable issued commercial version. If commercial scope must change before activation, the system creates a **new DRAFT quote revision** (clone), then sends/approves that revision; it does not mutate the issued quote in place. Execution planning remains operational and reviewable, but sold commercial truth is preserved in immutable issued versions and checkpoints.
